...
namespace JonnyGallo.Data
{
	public class SyncHandler<T> : IMobileServiceSyncHandler where T : IObservableEntityData
	{
		public async Task<JObject> ExecuteTableOperationAsync(IMobileServiceTableOperation operation)
		{
			...
			do
			{
				...
				if (preconditionFailedException != null)
				{
					...
					//var localItem = operation.Item.ToObject<JonnyGallo>();
					//RaiseDataSyncErrorEvent(new DataSyncErrorEventArgs<JonnyGallo>(localItem, serverItem));
					//operation.AbortPush();
					//return result;
				}
			} 
		...
		protected virtual void RaiseDataSyncErrorEvent(DataSyncErrorEventArgs<Dish> e)
		{
			DataSyncErrorEventHandler<Dish> handler = OnDataSyncError;
